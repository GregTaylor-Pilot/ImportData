...
Module mdlMain
    ...
    Private ShipFile, ShipSQL, ShipBak, ShipLog, SQLFile, ASNDataPath As String
    ...
    Private Sub MaskPrompts()
        ...
        Try
            ...
            For Each node As XmlNode In config.SelectNodes("/configuration/userSettings/ImportData.My.MySettings/setting")
                ...
                InterbaseQuery(querySql, queryFile, 10000)

                result = ""
                line = ""
                minSequence = "0"
                reader = New StreamReader(New FileStream(queryFile, FileMode.Open, FileAccess.Read))
                While Not reader.EndOfStream()
                ...
                If result <> "" Then
                    ...
                    If IsNumeric(pmntcode_id) Then
                        ...
                        InterbaseQuery(querySql, queryFile)

                        reader = New StreamReader(New FileStream(queryFile, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                        While Not reader.EndOfStream()
                            line = reader.ReadLine().Trim()
                            If line <> "" Then
                                result = line
    ...
    Private Sub CallCrossSiteFix()
        ...
        InterbaseQuery(FixCrossSiteFile, FixCrossSiteLog)
    ...
    Private Sub CallPilotMasking()
        ...
        InterbaseQuery(PilotMaskingFile, PilotMaskingLog)
    ...
    Private Sub CheckForPendingPricebatches()
        If mPendingPriceBatchesCheckTimes.Contains(Date.Now.Hour) Then
            ...
            Writer.WriteLine("select count(*) from pricemodhdr where status = 'P' and (valdate > ""today"" - 7) and (valdate <= ""today"");")
            Writer.Close()
            ProgressDialog.AddProgress("Checking for pending pricebatches")
            mLogger.Log("   Call pending pricebatches sql", 2)
            InterbaseQuery(PendingPricebatchesFile, PendingPricebatchesLog)
            mLogger.Log("   Call pending pricebatches sql ended", 3)
            File.Delete(PendingPricebatchesFile)

            Dim reader As New StreamReader(New FileStream(PendingPricebatchesLog, FileMode.Open, FileAccess.Read))
            Dim line, results As String
            results = ""
    ...
    Private Sub InterbaseQuery(ByVal source As String, ByVal destination As String, Optional ByVal timeout As Integer = 0)
        If timeout = 0 Then
    ...
    Private Function InterbaseQuery(ByVal query As String) As String
        ...
        Dim estring As New Odbc.OdbcConnectionStringBuilder("DRIVER=Firebird/InterBase(r) driver;UID=SYSDBA;PWD=masterkey;DBNAME=" & OfficeDB & ";")
        connection = New Odbc.OdbcConnection(estring.ToString)

        Dim adapter As New Odbc.OdbcDataAdapter(query, estring.ToString)
        ...
        Try
            ...
            connection.Dispose()
        Catch ex As Exception
            MessageBox.Show("Failed to retrieve data. Error: " & ex.Message)
            Return ""
        End Try

        If resultTable.Rows.Count > 0 Then
            Return resultTable.Rows(0).Item(0).ToString()
        End If
    ...
    Private Function PendingPostSales() As Boolean
        ...
        Try
            results = InterbaseQuery("select count(Postsales) from daybatch where PostSales = 'F';")
            If results <> "" Then
        ...
        While Not Success
            ...
            InterbaseQuery(countSQL, countResult)

            mLogger.Log("   Searching Database ended", 1)

            Sleep(1000)
            Reader = New StreamReader(New FileStream(countResult, FileMode.Open, FileAccess.Read))
    ...
    Private Sub UpdateItemStatus()
        ...
        InterbaseQuery(FixItFile, FixItLog)
